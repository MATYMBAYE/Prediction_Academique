"""Structure académique officielle d'ISI SUPTECH.

Domaines, Filières, Niveaux et Matières avec coefficients et typologies.
"""
...
def get_all_filieres():
    """Retourne la liste plate de toutes les filières avec leur domaine et niveaux autorisés."""
    items = []
    for dom_key, dom_data in DOMAINS_STRUCTURE.items():
        for code, f_data in dom_data["filieres"].items():
            items.append({
                "domaine_key": dom_key,
                "domaine_label": dom_data["label"],
                "code": code,
                "nom": f_data["nom"],
                "niveaux": list(f_data["niveaux"].keys()),
            })
    return items


def get_all_matieres():
    """Retourne la liste complète de toutes les matières avec filière, niveau, coefficient et type."""
    matieres = []
    for dom_key, dom_data in DOMAINS_STRUCTURE.items():
        for f_code, f_data in dom_data["filieres"].items():
            for niveau, mats in f_data["niveaux"].items():
                for m in mats:
                    matieres.append({
                        "domaine": dom_data["label"],
                        "filiere_code": f_code,
                        "filiere_nom": f_data["nom"],
                        "niveau": niveau,
                        "code": m["code"],
                        "nom": m["nom"],
                        "coeff": m.get("coeff", 1.0),
                        "type": m.get("type", "fondamentale"),
                    })
    return matieres
